...
def get_model(model_type, out_activation='relu'):
    if model_type == 'ahdr':
    ...
    elif model_type == 'adnet-group1':
        model = ADNet(n_channels=64, n_dense_layers=3, growth_rate=32, out_activation=out_activation, conv_groups=1)
        ...
        model = ResRefDANet.create('groups16', n_channels=64, out_activation=out_activation)
    else:
